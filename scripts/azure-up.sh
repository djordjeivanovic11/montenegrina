...
for key in AZURE_LOCATION AZURE_ENV_NAME POSTGRES_ADMIN_PASSWORD SESSION_SECRET INTERNAL_TOKEN_SECRET VOICE_AGENT_SERVICE_SECRET LIVEKIT_URL LIVEKIT_API_KEY LIVEKIT_API_SECRET OPENAI_API_KEY OPENAI_MODEL OPENAI_REALTIME_MODEL OPENAI_STT_MODEL OPENAI_TTS_MODEL OPENAI_TTS_VOICE VOICE_STT_PROVIDER VOICE_TTS_PROVIDER DEEPGRAM_API_KEY ELEVENLABS_API_KEY ELEVENLABS_MONTENEGRIN_VOICE_ID MNE_MCP_ENABLED MNE_MCP_API_URL MNE_MCP_API_KEY GOOGLE_CLIENT_ID PUBLIC_GOOGLE_CLIENT_ID; do
  azd env set "$key" "${!key}"
done
