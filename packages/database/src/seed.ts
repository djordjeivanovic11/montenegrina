...
try {
    ...
    if (existing) {
      ...
      await db.transaction(async (transaction) => {
        ...
        await transaction.insert(users).values({
          ...
          passwordHash: await argon2.hash(environment.LOCAL_ADMIN_PASSWORD, {
        ...
        await transaction.insert(providerConfigurations).values([
          ...
          },
          {
            ...
            settings: {
              voiceRef: 'env://ELEVENLABS_MONTENEGRIN_VOICE_ID',
        ...
        const toolRows = [
          [
            'product_lookup',
            'Lookup public product information',
            'READ_PUBLIC',
          ...
          [
            'appointment_create',
            'Create a reversible appointment',
            'WRITE_REVERSIBLE',
            'sandbox.appointment_create',
          ],
          [
            'request_status',
            'Look up a customer request',
            'READ_CUSTOMER',
  ...
  for (const planDef of planDefinitions) {
    ...
    process.stdout.write(`Seeded or updated plan ${planDef.slug}.\n`);
  }
} finally {
  await pool.end();
}
