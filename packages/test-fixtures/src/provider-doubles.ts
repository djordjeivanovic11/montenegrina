...
export class FakeSpeechToTextProvider implements SpeechToTextProvider {
  ...
  async transcribe(request: SpeechToTextRequest): Promise<ProviderResult<TranscriptionResult>> {
  ...
  async health(): Promise<{ healthy: boolean }> { return { healthy: true }; }
}

export class FakeLanguageModelProvider implements LanguageModelProvider {
  readonly id = 'fake-llm';
  async generate(request: LanguageModelRequest): Promise<ProviderResult<LanguageModelResult>> {
  ...
  async *stream(request: LanguageModelRequest, context: ProviderRequestContext): AsyncIterable<LanguageModelStreamEvent> {
...
function wav(pcm: Uint8Array, sampleRate: number): Uint8Array {
  ...
  result.set(new TextEncoder().encode('data'), 36); view.setUint32(40, pcm.length, true);
  result.set(pcm, 44);
  return result;
}
...
export class FakeTextToSpeechProvider implements TextToSpeechProvider {
  ...
  async synthesize(request: TextToSpeechRequest): Promise<ProviderResult<Uint8Array>> {
    const pcm = new Uint8Array(request.outputFormat.sampleRate / 2);
    const data = request.outputFormat.encoding === 'wav' ? wav(pcm, request.outputFormat.sampleRate) : pcm;
    return { data, metadata: meta(this.id, { characters: request.text.length }) };
  }
  async *stream(request: TextToSpeechRequest, context: ProviderRequestContext): AsyncIterable<AudioChunk> {
...
export class FakeEmbeddingProvider implements EmbeddingProvider {
  ...
  async embed(request: EmbeddingRequest): Promise<ProviderResult<number[][]>> {
    const data = request.texts.map((text) => {
      const digest = createHash('sha256').update(text.normalize('NFC')).digest();
      return Array.from({ length: this.dimensions }, (_, index) => ((digest[index % digest.length] ?? 0) - 127.5) / 127.5);
    });
...
class FakeRealtimeSession implements RealtimeSpeechSession {
  ...
  async interrupt(): Promise<void> { this.queue.splice(0, this.queue.length, { type: 'interrupted' }); }
  async close(): Promise<void> { this.closed = true; }
}

export class FakeRealtimeSpeechProvider implements RealtimeSpeechProvider {
  readonly id = 'fake-realtime';
  async connect(): Promise<RealtimeSpeechSession> { return new FakeRealtimeSession(); }
  async health(): Promise<{ healthy: boolean }> { return { healthy: true }; }
}
